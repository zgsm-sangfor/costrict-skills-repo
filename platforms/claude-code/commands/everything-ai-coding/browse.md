---
description: 'Browse coding resource categories. Usage: /everything-ai-coding:browse [category] [type:mcp|skill|rule|prompt]'
---
...
1. Download index to temp file: `curl -sf --compressed <index URL> -o "$TMPDIR/everything-ai-coding-index.json"`, on failure try Fallback URL, then use Read to load local fallback and save to the same temp file
...
data = json.load(open('$TMPDIR/everything-ai-coding-index.json'))
...
Footer: suggest using `/everything-ai-coding:browse <category>` for details; for verified recommendations, use `/everything-ai-coding:search` or `/everything-ai-coding:recommend`
