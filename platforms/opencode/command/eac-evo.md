---
description: 'Evolve a locally installed skill / prompt / rule. Usage: /eac-evo <id>'
...
> This resource is not installed. Run `/eac-install <id>` first, then come back to evo.
...
Use the project's LLM (same model already used for `/eac-install` customization flow). For claude-code, this is the live Claude conversation; no external API key needed.
...
Structure:
  ...
  - Next step: re-run `/eac-evo <id>` later to check remaining weak dimensions
```
