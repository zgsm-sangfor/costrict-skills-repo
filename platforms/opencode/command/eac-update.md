...
1. **Detect current platform**
   ...
   - Check if `~/.claude/skills/eac/SKILL.md` exists → Claude Code
...
2. **Download latest files**
   ...
   ```bash
   # Skill (global)
   curl -sfL "https://raw.githubusercontent.com/zgsm-ai/everything-ai-coding/main/platforms/claude-code/skills/eac/SKILL.md" -o ~/.claude/skills/eac/SKILL.md

   # Sub-commands (project-level)
   mkdir -p .claude/commands/eac/
   for cmd in search browse recommend install uninstall update; do
     curl -sfL "https://raw.githubusercontent.com/zgsm-ai/everything-ai-coding/main/platforms/claude-code/commands/eac/${cmd}.md" -o ".claude/commands/eac/${cmd}.md"
   done
   ```
...
3. **Report result** (in user's language)
   ...
   Structure:
     ...
     File list:
       - ~/.claude/skills/eac/SKILL.md
       - .claude/commands/eac/search.md
       - .claude/commands/eac/browse.md
       - .claude/commands/eac/recommend.md
       - .claude/commands/eac/install.md
       - .claude/commands/eac/uninstall.md
       - .claude/commands/eac/update.md
   ```
