...
### MCP (type == "mcp")
- 默认写入 `.cospec/settings.json`，用户选 "全局" 则写入 `~/.cospec/settings.json`
...
- 默认保存到 `.cospec/rules/<id>.md`，用户选 "全局" 则保存到 `~/.cospec/rules/<id>.md`
- 如果是 .cursorrules 格式，保持原文本内容

### Prompt (type == "prompt")
- 同 Rule 的安装逻辑
- 保存到 `.cospec/rules/<id>.md`
